...
class docx_generator {
    ...
    public static function generate_student_docx(array $items, string $title, string $course_name): string {
        $total_points = 0;
    ...
    public static function generate_teacher_docx(array $items, string $title, string $course_name, string $justification, array $objectives_data): string {
        $total_points = 0;
        foreach ($items as $item) {
            $total_points += (float)($item['points'] ?? 0);
        }
